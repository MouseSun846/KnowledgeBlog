...
<h2 id="资源短缺" tabindex="-1"><a class="header-anchor" href="#资源短缺"><span>资源短缺</span></a></h2>
<p>QoS 划分的主要应用场景，是当宿主机资源紧张的时候，kubelet 对 Pod 进行 Eviction（即资源回收）时需要用到的。</p>
...
<div class="language- line-numbers-mode" data-highlighter="shiki" data-ext="" data-title="" style="--shiki-light:#24292e;--shiki-dark:#abb2bf;--shiki-light-bg:#fff;--shiki-dark-bg:#282c34"><pre v-pre class="shiki shiki-themes github-light one-dark-pro vp-code"><code><span class="line"><span>memory.available&#x3C;100Mi</span></span>
<span class="line"><span>nodefs.available&#x3C;10%</span></span>
<span class="line"><span>nodefs.inodesFree&#x3C;5%</span></span>
<span class="line"><span>imagefs.available&#x3C;15%</span></span></code></pre>
<div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>上述各个触发条件在 kubelet 里都是可配置的</p>
<div class="language- line-numbers-mode" data-highlighter="shiki" data-ext="" data-title="" style="--shiki-light:#24292e;--shiki-dark:#abb2bf;--shiki-light-bg:#fff;--shiki-dark-bg:#282c34"><pre v-pre class="shiki shiki-themes github-light one-dark-pro vp-code"><code><span class="line"><span>kubelet --eviction-hard=imagefs.available&#x3C;10%,memory.available&#x3C;500Mi,nodefs.available&#x3C;5%,nodefs.inodesFree&#x3C;5% --eviction-soft=imagefs.available&#x3C;30%,nodefs.available&#x3C;10% --eviction-soft-grace-period=imagefs.available=2m,nodefs.available=2m --eviction-max-pod-grace-period=600</span></span></code></pre>
<div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0"><div class="line-number"></div></div></div><p>Eviction 在 Kubernetes 里其实分为 Soft 和 Hard 两种模式。</p>
...
<p>Pod 的 QoS 类别：  Guaranteed &gt; Burstable &gt; BestEffort</p>
<p>Kubernetes 会保证只有当 Guaranteed 类别的 Pod 的资源使用量超过了其 limits 的限制，或者宿主机本身正处于 Memory Pressure 状态时，Guaranteed 的 Pod 才可能被选中进行 Eviction 操作。</p>
<h2 id="如何能够让-kubernetes-的调度器尽可能地将-pod-分布在不同机器上-避免-堆叠-呢" tabindex="-1"><a class="header-anchor" href="#如何能够让-kubernetes-的调度器尽可能地将-pod-分布在不同机器上-避免-堆叠-呢"><span>如何能够让 Kubernetes 的调度器尽可能地将 Pod 分布在不同机器上，避免“堆叠”呢?</span></a></h2>
<p>在 Kubernetes 中，可以通过以下几种方式配置调度策略，以尽可能地将 Pod 分布在不同的节点上，避免“堆叠”：</p>
<h3 id="_1-pod-反亲和性-pod-anti-affinity" tabindex="-1"><a class="header-anchor" href="#_1-pod-反亲和性-pod-anti-affinity"><span>1. Pod 反亲和性（Pod Anti-Affinity）</span></a></h3>
<p>Pod 反亲和性是一种调度约束，允许用户指定某些 Pod 不应该与其他特定 Pod 运行在同一个节点上。</p>
...
<span class="line"><span style="--shiki-light:#22863A;--shiki-dark:#E06C75">          topologyKey</span><span style="--shiki-light:#24292E;--shiki-dark:#ABB2BF">: </span><span style="--shiki-light:#032F62;--shiki-dark:#98C379">"kubernetes.io/hostname"</span></span></code></pre>
<div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>在这个示例中，<code v-pre>podAntiAffinity</code> 指定了具有相同标签 <code v-pre>app=myapp</code> 的 Pod 不应该被调度到相同的节点上。<code v-pre>topologyKey</code> 为 <code v-pre>kubernetes.io/hostname</code>，表示约束作用在节点级别。</p>
<h3 id="_2-节点亲和性-node-affinity" tabindex="-1"><a class="header-anchor" href="#_2-节点亲和性-node-affinity"><span>2. 节点亲和性（Node Affinity）</span></a></h3>
<p>Node Affinity 允许调度器根据节点标签选择合适的节点。这可以用于避免将所有 Pod 调度到相同的节点。</p>
...
<span class="line"><span style="--shiki-light:#22863A;--shiki-dark:#E06C75">            operator</span><span style="--shiki-light:#24292E;--shiki-dark:#ABB2BF">: </span><span style="--shiki-light:#032F62;--shiki-dark:#98C379">In</span></span>
<span class="line"><span style="--shiki-light:#22863A;--shiki-dark:#E06C75">            values</span><span style="--shiki-light:#24292E;--shiki-dark:#ABB2BF">:</span></span>
<span class="line"><span style="--shiki-light:#24292E;--shiki-dark:#ABB2BF">            - </span><span style="--shiki-light:#032F62;--shiki-dark:#98C379">ssd</span></span></code></pre>
<div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>这个示例展示了如何使用节点标签（<code v-pre>disktype=ssd</code>）进行调度，以确保 Pod 被调度到带有特定标签的节点上。</p>
<h3 id="_3-分布式调度策略-spread-constraints" tabindex="-1"><a class="header-anchor" href="#_3-分布式调度策略-spread-constraints"><span>3. 分布式调度策略（Spread Constraints）</span></a></h3>
<p>Kubernetes 1.18 引入了 <code v-pre>TopologySpreadConstraints</code>，允许用户定义分布式调度策略，确保 Pod 均匀地分布在集群的不同节点上。</p>
...
<span class="line"><span style="--shiki-light:#22863A;--shiki-dark:#E06C75">  containers</span><span style="--shiki-light:#24292E;--shiki-dark:#ABB2BF">:</span></span>
<span class="line"><span style="--shiki-light:#24292E;--shiki-dark:#ABB2BF">  - </span><span style="--shiki-light:#22863A;--shiki-dark:#E06C75">name</span><span style="--shiki-light:#24292E;--shiki-dark:#ABB2BF">: </span><span style="--shiki-light:#032F62;--shiki-dark:#98C379">my-container</span></span>
<span class="line"><span style="--shiki-light:#22863A;--shiki-dark:#E06C75">    image</span><span style="--shiki-light:#24292E;--shiki-dark:#ABB2BF">: </span><span style="--shiki-light:#032F62;--shiki-dark:#98C379">my-image</span></span></code></pre>
<div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>在这个示例中，<code v-pre>topologySpreadConstraints</code> 指定 Pod 应该均匀地分布在不同的节点上。<code v-pre>maxSkew</code> 表示同一节点上的 Pod 数量与其他节点上的最大偏差不超过1。</p>
<h3 id="_4-自定义调度器策略-custom-scheduler-policies" tabindex="-1"><a class="header-anchor" href="#_4-自定义调度器策略-custom-scheduler-policies"><span>4. 自定义调度器策略（Custom Scheduler Policies）</span></a></h3>
<p>Kubernetes 允许使用自定义调度策略文件来自定义调度行为。例如，可以设置 <code v-pre>EvenPodsSpreadPriority</code> 来实现均匀调度。</p>
<h4 id="示例-3" tabindex="-1"><a class="header-anchor" href="#示例-3"><span>示例</span></a></h4>
<p>自定义调度策略文件（<code v-pre>scheduler-policy-config.json</code>）：</p>
...
<div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>启动调度器时使用该策略文件：</p>
<div class="language-shell line-numbers-mode" data-highlighter="shiki" data-ext="shell" data-title="shell" style="--shiki-light:#24292e;--shiki-dark:#abb2bf;--shiki-light-bg:#fff;--shiki-dark-bg:#282c34"><pre v-pre class="shiki shiki-themes github-light one-dark-pro vp-code"><code><span class="line"><span style="--shiki-light:#6F42C1;--shiki-dark:#61AFEF">kube-scheduler</span><span style="--shiki-light:#005CC5;--shiki-dark:#D19A66"> --policy-config-file=scheduler-policy-config.json</span></span></code></pre>
<div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0"><div class="line-number"></div></div></div><p>通过配置这些策略，可以显著改善 Pod 在集群中的分布情况，避免“堆叠”问题，实现资源的更高效利用。</p>
<h2 id="kubelet如何实现-exec、logs-等接口" tabindex="-1"><a class="header-anchor" href="#kubelet如何实现-exec、logs-等接口"><span>kubelet如何实现 exec、logs 等接口</span></a></h2>
<p>gRPC 接口调用期间，kubelet 需要跟容器项目维护一个长连接来传输数据。这种 API，我们就称之为 Streaming API。</p>
<p>CRI shim 里对 Streaming API 的实现，依赖于一套独立的 Streaming Server 机制。</p>
<figure><img src="/assets/images/693e0a87-d9f1-4fe5-a733-79e618b922d2.png" alt="" tabindex="0" loading="lazy"><figcaption></figcaption></figure>
...
<h2 id="集群安装" tabindex="-1"><a class="header-anchor" href="#集群安装"><span>集群安装</span></a></h2>
<p>kubeadm init --cri-socket unix:///var/run/cri-dockerd.sock</p>
...
<span class="line"><span>Jul 29 15:07:31 DESKTOP-P54EAF3 systemd[1]: kubelet.service: Failed with result 'exit-code'.</span></span></code></pre>
