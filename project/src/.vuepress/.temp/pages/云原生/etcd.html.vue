...
<p class="hint-container-title">未获得锁的 client 是如何等待的呢?</p>
...
<div class="hint-container tip">
<p class="hint-container-title">为什么使用etcd分布式锁比redis分布式锁更好？</p>
<p>相比 Redis 基于主备异步复制导致锁的安全性问题，etcd 是基于 Raft 共识算法实现的，一个写请求需要经过集群多数节点确认。因此一旦分布式锁申请返回给 client 成功后，它一定是持久化到了集群多数节点上，不会出现 Redis 主备异步复制可能导致丢数据的问题，具备更高的安全性。</p>
</div>
<div class="hint-container tip">
<p class="hint-container-title">分布式锁的三个主要核心要素</p>
<p>安全性、互斥性。在同一时间内，不允许多个 client 同时获得锁。</p>
<p>活性。无论 client 出现 crash 还是遭遇网络分区，你都需要确保任意故障场景下，都不会出现死锁，常用的解决方案是超时和自动过期机制。</p>
<p>高可用、高性能。加锁、释放锁的过程性能开销要尽量低，同时要保证高可用，避免单点故障。</p>
</div>
</div></template>
