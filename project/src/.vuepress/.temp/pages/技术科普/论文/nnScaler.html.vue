...
<h3 id="_7-实现与经验" tabindex="-1"><a class="header-anchor" href="#_7-实现与经验"><span>7. 实现与经验</span></a></h3>
<p>我们基于 PyTorch 实现了 nnScaler，代码量达到了 24,000 行 Python 代码。nnScaler 读取为单设备开发的 PyTorch 模型，并将其转换为中间图表示 (IR)。在进行转换、时空调度以及插入通信和张量操作后，每个设备将接收一个由 IR 表示的子图。nnScaler 然后将这些子图转换回 PyTorch 代码文件，并通过 <code v-pre>torchrun</code> 并行执行以进行分布式训练。</p>
<p>为了支持各种 PyTorch 模型，nnScaler 基于 TorchFX 实现了增强的图转换器，包含 2,243 行 Python 代码。该转换器结合了 TorchFX 的符号执行和 <code v-pre>torch.jit.trace</code> 的值跟踪来处理控制流，这是将 PyTorch 模型转换为 TorchFX 时的典型障碍。默认情况下，PyTorch 模型通常只包含前向传播，nnScaler 自动使用链式法则的 <code v-pre>autograd</code> 功能完成反向传播。到目前为止，nnScaler 已成功转换了 HuggingFace 自然语言处理任务中的 84.1% 的 PyTorch 模型。转换失败的原因主要是由于某些不支持的算子（例如为特定模型设计的自定义算子）。我们正在积极探索支持更多算子及其对应的转换算法的方法。</p>
<h4 id="_7-1-实践经验" tabindex="-1"><a class="header-anchor" href="#_7-1-实践经验"><span>7.1 实践经验</span></a></h4>
<p>nnScaler 已被 Microsoft 不同团队的多个项目使用，用于支持多代 NVIDIA 和 AMD GPU 上的下一代 DNN 模型的预训练和微调。这些模型包括 RetNet、YOCO、LongRoPE、Phi-3 系列以及一个由 Transformer 和图神经网络（GNN）组成的大型科学基础模型，模型参数范围从 30 亿到 920 亿不等。</p>
<p>选择使用 nnScaler 的决定主要基于两个关键因素。首先，将新模型整合到现有的分布式训练框架中通常会遇到复杂的工程挑战，包括新模块的并行化、适当的划分选项以及确保端到端训练的正确性。这个过程通常需要两名有经验的工程师大约两个月的时间才能完成。此外，现有的并行化方案往往在新模型上表现不佳，导致模型 FLOPs 利用率 (MFU) 不理想。其次，研究新模型通常需要更改模型架构、配置和训练设置，这可能需要进一步调整并行化计划以提高训练效率。nnScaler 正是为了解决这些问题而设计的。</p>
<p>此外，我们与这些团队的合作也带来了许多启示。</p>
<h4 id="_7-1-1-调试-nnscaler" tabindex="-1"><a class="header-anchor" href="#_7-1-1-调试-nnscaler"><span>7.1.1 调试 nnScaler</span></a></h4>
<p>nnScaler 在模型训练中提供了很大的灵活性，但新的原语和约束也增加了系统的复杂性，使某些并行化计划容易出错。为了解决这个问题，nnScaler 采用了一种模块化的调试方法，允许将由新约束生成的较少研究的子图替换为经过充分测试的约束。例如，nnScaler 可以选择性地将数据并行性应用于模型的一部分，而其余部分保持现有的并行化计划不变。此调整不需要修改模型代码，只需重新配置预构建的并行化计划。通过迭代地更改计划中疑似有问题的模块，这种方法有助于定位问题模块。</p>
<h4 id="_7-1-2-模型准确性" tabindex="-1"><a class="header-anchor" href="#_7-1-2-模型准确性"><span>7.1.2 模型准确性</span></a></h4>
<p>实现高模型准确性是训练的最终目标。然而，有时训练框架或模型代码中的一个小错误就会导致准确性下降。更复杂的是，在训练早期阶段，系统可能表现正常，但经过数千步的训练后，损失曲线可能恶化，最终出现发散。直接将损失和梯度值与经过充分测试的训练计划（如数据并行性）进行比较是不现实的。例如，在更复杂的计划中，诸如矩阵乘法 (matmul) 或全归约 (allreduce) 操作会因不同的加法顺序导致浮点值漂移，这是预期的行为。这使得很难分辨是预期中的数值偏差，还是语义错误。</p>
<p>为了解决这个问题，nnScaler 首先通过减少模型的隐藏维度，使其适应单个设备的训练，从而简化调试过程。由于模型代码与训练代码之间的清晰分离，这种模型更改可以很容易实现。接下来，我们将搜索到的并行化计划应用于简化后的模型，并通过比较损失曲线和梯度归一化曲线来评估其与数据并行训练的重叠情况。我们观察到，梯度归一化曲线是一个良好的指标，能够在早期放大偏差并预示系统中潜在的错误。</p>
<h4 id="_7-1-3-就地操作符" tabindex="-1"><a class="header-anchor" href="#_7-1-3-就地操作符"><span>7.1.3 就地操作符</span></a></h4>
<p>为了提高训练性能，像 <code v-pre>Tensor.add_</code> 这样的就地操作符会在原地更新张量。然而，对就地操作符进行划分可能会带来问题。例如，如果就地操作符的划分导致张量被克隆，而原本应在原地更新的操作变成了非就地操作，结果就不会保留原来就地操作符的效果。这是因为在混合使用就地操作符和非就地操作符时，会违反静态单一赋值 (SSA) 形式。为了避免这个问题，nnScaler 在图转换过程中遵循 SSA 规则，并在后期优化阶段将某些非就地操作符替换为原始的就地版本。</p>
<h3 id="_8-评估" tabindex="-1"><a class="header-anchor" href="#_8-评估"><span>8. 评估</span></a></h3>
<p>对 nnScaler 的评估涵盖了并行化原语的表达能力以及带有约束的并行化计划搜索效率。更重要的是，我们对实际模型的性能进行了评估，以展示整个系统在实现新模型和设置的高效并行化方面的有效性。总结来说，评估结果表明：</p>
<ul>
<li>nnScaler 中的并行化原语可以构建各种并行化计划，包括现有的手工设计的计划（§8.1）和本文中介绍的创新计划（§8.2）。</li>
<li>对 SwinTransformer、T5 和 AlphaFold2 的三个新并行化计划进行的端到端评估，显示它们相较于 Megatron-LM 、Alpa 、DeepSpeed 和 DAP 基线方案分别加速了 3.5×、2.5× 和 1.4×（§8.3）。</li>
<li>带有约束的并行化空间帮助 nnScaler 快速发现高效计划，搜索速度提高了 11.7×，相比于无约束的搜索。</li>
</ul>
<h4 id="_8-1-计划构建的表达能力" tabindex="-1"><a class="header-anchor" href="#_8-1-计划构建的表达能力"><span>8.1 计划构建的表达能力</span></a></h4>
<p>我们通过实现表 8 中列出的流行手工并行化计划，评估了三种原语在计划构建中的表达能力。这些计划可以分解为操作符转换、放置和排序，与表 1 中的三种原语紧密对齐。nnScaler 成功支持了 17 个并行化计划中的 14 个。</p>
<p>在 SPMD（单程序多数据）下的并行化计划是通过操作符转换实现的。数据并行性和灵活的张量并行性可以轻松支持。Transformer 并行性和 DAP 是为 Transformer 和 AlphaFold2 手工设计的张量并行性。序列并行性和 ZeRO stage-3 是特殊的张量并行性，它们解耦了操作符及其输入张量的划分，以优化内存使用。nnScaler 通过在输入张量和其操作符之间插入一个恒等操作符，通过操作符转换轻松实现了这种解耦。</p>
<p>MPMD（多程序多数据）下的并行化计划是不同类型的手工流水线并行化。它们可以使用操作符排序来支持，无需实现新的执行引擎。值得注意的是，nnScaler 不支持 PipeDream，因为它采用的是异步训练方法，而 nnScaler 尊重模型的原始训练语义。对于 TeraPipe，nnScaler 目前缺乏访问张量中具体值的能力，因此无法在令牌级别确定数据依赖关系（即张量掩码），这是 TeraPipe 的一个要求。未来，nnScaler 可以通过深度学习模型的检测工具来实现 TeraPipe。</p>
<p>除了并行化之外，nnScaler 还支持内存优化技术（如重计算、交换）和计算与通信的重叠。对重计算的支持依赖于操作符转换的自定义算法 .</p>
<h4 id="_8-2-计划搜索结果" tabindex="-1"><a class="header-anchor" href="#_8-2-计划搜索结果"><span>8.2 计划搜索结果</span></a></h4>
<p>8.2 计划搜索结果</p>
<p>根据第 4.2 节描述的新约束，nnScaler 在每个构建的空间内进行搜索，并发现了三种在训练性能上表现优越的并行化计划。</p>
<p><strong>Coshard</strong>：图 5 展示了 Coshard 计划，适用于像 SwinTransformer 这样包含大张量的模型。它可以与张量并行共存，以减少激活张量的峰值内存。例如，A1 被分割成两部分，放置在同一个设备上，并顺序执行。应用 A1 的重计算后，A1 的峰值激活大小减少了一半。由于峰值内存的减少，张量并行现在可以跨越更少的设备（例如，从 8 路减少到 4 路），从而降低通信成本。</p>
<p><strong>交错流水线</strong>：图 6 显示了在表 6 中指定约束下搜索到的流水线调度。嵌入层通过张量并行划分到四个设备上，剩余的非嵌入层组件则根据 <code v-pre>staged_spmd</code> 分配到不同的设备组。在排序搜索过程中，所有层组合成一个调度，该调度类似于执行嵌入层和 1F1B（一次正向传播一次反向传播）的时间共享模式。由于调度搜索，流水线能够在没有“气泡”的情况下达到稳定状态，如图右侧所示。</p>
<p><strong>3F1B 流水线</strong>：图 2 展示了第 4.2 节描述的 3F1B 流水线的时间轴。表 7 中概述的约束定义了正向和反向传递如何在流水线的稳定状态中交错执行。</p>
<h4 id="_8-3-端到端性能" tabindex="-1"><a class="header-anchor" href="#_8-3-端到端性能"><span>8.3 端到端性能</span></a></h4>
<p>我们分别在 SwinTransformer、T5 和 AlphaFold2 上对三种新并行化计划进行了评估，涵盖不同的模型配置和 GPU 数量。</p>
<h5 id="_8-3-1-实验设置" tabindex="-1"><a class="header-anchor" href="#_8-3-1-实验设置"><span>8.3.1 实验设置</span></a></h5>
<p><strong>机器配置</strong>：我们的评估在配备 32 个 NVIDIA Tesla V100（32GB）的 DGX-2 集群上进行。每台服务器配备 16 个 GPU，使用 NVLink 互联。服务器之间通过 8 个 100 Gbps 的 InfiniBand 网络适配器互联。所有服务器均安装了 NCCL 2.14 和 PyTorch v2.0.1。</p>
<p><strong>模型配置</strong>：我们在 SwinTransformer、T5 和 AlphaFold2 的四种不同模型配置下进行实验，从小模型到大模型不等。每个配置都包含参数数量、层数、隐藏维度和头数。</p>
<p><strong>基线系统</strong>：我们将 nnScaler 与三个流行的分布式训练系统进行了比较：</p>
<ol>
<li>
<p><strong>Megatron-LM</strong>：专为训练基于 Transformer 的模型设计，结合了流水线并行、数据并行和张量并行的三维并行化方式。对于流水线并行，它将模型层均匀划分为多个阶段，每个阶段可以进一步应用数据并行和张量并行。当 GPU 数量足够多时，Megatron-LM 还可以使用 <span v-pre class="katex"><span class="katex-mathml"><math xmlns="http://www.w3.org/1998/Math/MathML"><semantics><mrow><mfrac><mi>N</mi><mrow><mi>M</mi><mo>⋅</mo><mi>K</mi></mrow></mfrac></mrow><annotation encoding="application/x-tex">{\frac{N}{M \cdot K}}</annotation></semantics></math></span><span class="katex-html" aria-hidden="true"><span class="base"><span class="strut" style="height:1.2173em;vertical-align:-0.345em;"></span><span class="mord"><span class="mord"><span class="mopen nulldelimiter"></span><span class="mfrac"><span class="vlist-t vlist-t2"><span class="vlist-r"><span class="vlist" style="height:0.8723em;"><span style="top:-2.655em;"><span class="pstrut" style="height:3em;"></span><span class="sizing reset-size6 size3 mtight"><span class="mord mtight"><span class="mord mathnormal mtight" style="margin-right:0.10903em;">M</span><span class="mbin mtight">⋅</span><span class="mord mathnormal mtight" style="margin-right:0.07153em;">K</span></span></span></span><span style="top:-3.23em;"><span class="pstrut" style="height:3em;"></span><span class="frac-line" style="border-bottom-width:0.04em;"></span></span><span style="top:-3.394em;"><span class="pstrut" style="height:3em;"></span><span class="sizing reset-size6 size3 mtight"><span class="mord mtight"><span class="mord mathnormal mtight" style="margin-right:0.10903em;">N</span></span></span></span></span><span class="vlist-s">​</span></span><span class="vlist-r"><span class="vlist" style="height:0.345em;"><span></span></span></span></span></span><span class="mclose nulldelimiter"></span></span></span></span></span></span>路数据并行来进一步提高训练性能。</p>
</li>
<li>
<p><strong>Alpa</strong>：一个针对深度学习模型的自动并行化系统，基于 TensorFlow，使用三维并行化空间。它的搜索算法和训练系统目前基于 TensorFlow。为了进行直接比较，我们在 nnScaler 中实现了 Alpa 的搜索算法，作为一种策略。</p>
</li>
<li>
<p><strong>DeepSpeed</strong>：与 Megatron-LM 类似的分布式训练系统，支持流水线并行、数据并行和张量并行。此外，DeepSpeed 集成了 ZeRO 和 ZeRO-Offload 等技术，以优化 GPU 内存使用。ZeRO 主要通过在数据并行模式下只保留优化器状态的单份拷贝来减少内存占用，而 ZeRO-Offload 则将权重卸载到 CPU 内存，从而减轻 GPU 的内存压力，并在需要时将其取回。</p>
</li>
</ol>
<p>Megatron-LM 和 DeepSpeed 都不具备在其支持的并行化空间内自动搜索并行化计划的功能。因此，我们通过分别遍历流水线并行、张量并行和数据并行的不同程度，手动找到了它们的最佳性能计划。在接下来的所有实验中，我们应用了逐层重计算【11】，以减少激活张量的内存消耗。按照常见做法【29, 65】，我们使用了累计的有效 TFLOPS 作为性能指标。</p>
<h5 id="_8-3-2-swintransformer-的结果" tabindex="-1"><a class="header-anchor" href="#_8-3-2-swintransformer-的结果"><span>8.3.2 SwinTransformer 的结果</span></a></h5>
<p>图 7 展示了 SwinTransformer 在四个系统上的端到端训练吞吐量。由于激活张量的体积巨大（例如，一个 50 亿参数模型的第一个 Transformer 层的激活张量占用 21GB），即使应用了重计算，Megatron-LM 和 Alpa 也对所有模型配置使用了纯张量并行性。DeepSpeed 采用了 ZeRO-Offload 和 ZeRO stage 3 来优化内存使用，因此它能够在 4 个 GPU 设置中应用 2 路张量并行性，并在其余三种设置中应用 4 路张量并行性。为了扩展到所有可用的 GPU，还进一步应用了数据并行性。nnScaler 在 SwinTransformer 的前四层（Attention + MLP）上应用了 Coshard 计划，因为这些层由于激活张量占用了大量内存。nnScaler 分别对四种配置应用了 2 路、2 路、4 路和 8 路张量并行性，并分别结合了 2 路、4 路、4 路和 4 路流水线并行性。在 8 路 GPU 设置中，Coshard 对每个 GPU 顺序执行 6 个分区，而在其余三个设置中执行 4 个分区。</p>
<p>如图 7 所示，nnScaler 在 8、16 和 32 个 GPU 上的速度分别比 DeepSpeed 快 1.2 倍、1.5 倍和 1.5 倍。</p>
<h5 id="_8-3-3-t5-的结果" tabindex="-1"><a class="header-anchor" href="#_8-3-3-t5-的结果"><span>8.3.3 T5 的结果</span></a></h5>
<p>图 8 展示了 T5 的端到端训练吞吐量。对于 4 个 GPU，Megatron-LM 使用了 2 路张量并行性和 2 路流水线并行性，而对于 8、16 和 32 个 GPU，它只使用了纯张量并行性。Alpa 对于 4 个 GPU 使用了 3 路流水线并行性，且中间阶段应用了 2 路张量并行性。由于内存消耗较大，Alpa 必须在 8、16 和 32 个 GPU 上使用纯张量并行性。因为 T5 模型参数较小（3.9B），DeepSpeed 在 4 个 GPU 上可以使用数据并行性结合 ZeRO stage 3。在 8、16 和 32 个 GPU 上，它采用了 4 路张量并行性，并结合了 ZeRO-Offload 和 ZeRO stage 3。此外，还进一步应用了数据并行性扩展到所有可用 GPU。nnScaler 则应用了交错流水线，使用张量并行性处理大嵌入层，其他层则通过 4 路流水线并行性进行处理，每个阶段分别应用了 1 路、2 路、4 路和 8 路张量并行性，分别对应 4、8、16 和 32 个 GPU。</p>
<p>nnScaler 在 8、16 和 32 个 GPU 上的表现分别比 DeepSpeed 快 1.5 倍、1.6 倍和 2.5 倍。</p>
<h5 id="_8-3-4-alphafold2-的结果" tabindex="-1"><a class="header-anchor" href="#_8-3-4-alphafold2-的结果"><span>8.3.4 AlphaFold2 的结果</span></a></h5>
<p>图 9 显示了 AlphaFold2 的端到端训练吞吐量。在这个实验中，我们与两个基线进行了比较。一个是专门为 AlphaFold2 设计的手工张量并行方案 DAP（Deep Analysis Pipeline）。我们还应用了数据并行性来扩展 DAP，称为 DAP+DP。对于 4 和 8 个 GPU，DAP+DP 使用了纯数据并行性，因为模型较小。对于 16 个 GPU，它使用了 4 路张量并行性结合 4 路数据并行性。另一个基线是 DeepSpeed。由于 AlphaFold2 的模型尺寸比 SwinTransformer 和 T5 小得多，不需要使用 ZeRO-Offload。DeepSpeed 对于 4、8 和 16 个 GPU 使用了纯数据并行性结合 ZeRO stage 3，对于 32 个 GPU 使用了 2 路张量并行性和 16 路数据并行性。nnScaler 也在 4 和 8 个 GPU 上使用了纯数据并行性，而在 16 和 32 个 GPU 上应用了 3F1B 流水线。对于 16 个 GPU，nnScaler 使用了 4 路流水线并行性结合 4 路数据并行性；对于 32 个 GPU，它使用了 2 路张量并行性、2 路流水线并行性和 8 路数据并行性。</p>
<p>nnScaler 在 16 个 GPU 上比 DAP+DP 提高了 1.5 倍的性能，在 32 个 GPU 上比 DeepSpeed 提高了 1.1 倍的性能。</p>
<h5 id="_8-3-5-性能较低硬件上的实验" tabindex="-1"><a class="header-anchor" href="#_8-3-5-性能较低硬件上的实验"><span>8.3.5 性能较低硬件上的实验</span></a></h5>
<p>为了验证新并行化计划的有效性，并了解不同硬件对训练性能的影响，我们在 DGX-1 集群上对 SwinTransformer 和 AlphaFold2 进行了评估。图 10a 显示了 nnScaler 在 16 和 32 个 GPU 上分别比 DeepSpeed 快 1.9 倍和 3.5 倍。相比于图 7 中的数据，对于 32 个 GPU，nnScaler 的性能下降了 6%，而 DeepSpeed、Alpa 和 Megatron-LM 的性能分别下降了 60%、82% 和 82%。nnScaler 的性能下降较小，因为它使用的 Coshard 并行化计划优化了通信成本，能够适应通信带宽的变化。图 10b 显示了 AlphaFold2 在 DGX-1 上的结果。nnScaler 相对于 DeepSpeed 的相对性能增益在 16 和 32 个 GPU 上分别提高到 1.1 倍和 1.4 倍。</p>
<h4 id="_8-4-带有约束的搜索效率" tabindex="-1"><a class="header-anchor" href="#_8-4-带有约束的搜索效率"><span>8.4 带有约束的搜索效率</span></a></h4>
<p>算法 1 表明，nnScaler 中并行化计划的搜索成本包括：（1）操作符转换和放置的成本（即算法 1 中的第 1-4 行），以及（2）操作符时间排序的成本（即第 5 行）。图 11 展示了端到端搜索成本以及三种自定义空间（定义于第 4.2 节）的搜索时间细分。对于不同的模型配置，搜索策略如第 5 节所述。SwinTransformer 的搜索时间小于 150 秒。随着模型尺寸增加，操作符数量增加，搜索时间也相应增加。T5 的排序搜索时间约为 150 秒，因为 T5 的空间中没有约束条件。对于 SwinTransformer 和 AlphaFold2 来说，几乎没有排序搜索的成本。对于 SwinTransformer，顺序基本上由数据依赖关系决定；对于 AlphaFold2，排序约束大大减少了搜索空间。</p>
<p>图 12 进一步展示了 3F1B 调度在有约束和无约束情况下的时间排序搜索时间。左图显示，随着阶段数量的增加，搜索时间呈指数级增长。然而，应用约束后，搜索时间保持在 60 秒以内，从而在 4 个阶段的时间排序中实现了 11.7 倍的加速。这归因于表 7 中的时间排序约束，明确规定了来自不同微批次的独立正向和反向操作符的顺序，从而显著减少了搜索算法（如 Tessel）需要处理的搜索空间。</p>
<p>对于 4 个阶段的情况，右图进一步展示了每个来自表 7 的排序约束逐个应用时的搜索时间。第一个约束将搜索时间减少了 100 秒，第二个约束进一步将搜索时间减少了 50%。这表明了约束的重要性。</p>
</div></template>
